...
- About remains hidden while `SHOW_BEARER` is off; Firelink now always renders.
...
- Removed em dashes from source, documentation, metadata, and changelog text.
- Replaced duplicated Forged-tab project data with a derived view of the
  canonical PROJECTS array.
- Shortened project-card summaries to concise, readable descriptions.
- Delayed ambient effects until after the preloader closes.
- Replaced preloader intervals with one requestAnimationFrame timeline.
- Paused resume bloom and timeline GIF transforms while their sections are
  outside the viewport.
- Added an accessible label to the contact dialog close button.
- Rewrote README.md to match the current project structure and behavior.
