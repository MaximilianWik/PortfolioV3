    ...
    <>
      <nav className="flex justify-between items-center px-6 md:px-12 py-4 border-b border-bone-faded/30 sticky top-0 z-[90] bg-ink-void/90 backdrop-blur-sm">
        ...
        <div className="hidden md:flex gap-6 lg:gap-8 font-subdisplay text-[10px] tracking-widest text-bone-dim">
          {NAV_LINKS.map(l => (
            <a key={l.href} href={l.href} className="hover:text-gilt transition-colors">
              {l.label}
            </a>
          ))}
        </div>
            ...
            <nav className="flex flex-col items-center gap-8" onClick={e => e.stopPropagation()}>
              {NAV_LINKS.map((l, i) => (
                <motion.a
                  ...
                  onClick={close}
                  className="font-subdisplay text-sm tracking-[0.3em] text-bone-dim hover:text-gilt transition-colors uppercase"
                >
                  {l.label}
                </motion.a>
              ))}
