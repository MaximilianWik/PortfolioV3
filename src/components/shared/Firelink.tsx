  ...
  return (
    <section ref={containerRef} className="relative h-[60vh] md:h-[80vh] w-full overflow-hidden flex items-center justify-center my-32">
      ...
      <motion.div style={{ y }} className="absolute inset-0 w-full h-[140%] -top-[20%]">
        ...
        {/* Blurred duplicate is masked to the perimeter. The centre image stays crisp. */}
        <img
          src={firekeeperImg}
          alt=""
          aria-hidden="true"
          loading="lazy"
          decoding="async"
          className="absolute inset-0 w-full h-full object-cover object-center opacity-30 mix-blend-luminosity grayscale-[50%] pointer-events-none"
          style={{
            filter: 'blur(clamp(3px, 0.45vw, 9px))',
            transform: 'scale(1.03)',
            maskImage: 'radial-gradient(ellipse at center, transparent 56%, black 100%)',
            WebkitMaskImage: 'radial-gradient(ellipse at center, transparent 56%, black 100%)',
          }}
        />

        {/* Edge fades match the Hero bonfire treatment and hide crop boundaries. */}
