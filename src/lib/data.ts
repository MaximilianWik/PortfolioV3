...
export const PROJECTS = [
  {
    ...
    subtitle: "A browser-only computer vision cockpit that runs three MediaPipe Tasks Vision models (face, pose, hand) against the webcam entirely on-device, no backend, no upload, no telemetry. A hand-rolled scoring and stabilization pipeline classifies each frame into one of sixteen states — blank stare, side profile, tongue out, happy, kiss, angry, disgust, hands up, and seven hand shapes including heart hands — and renders the result through a fake retro IDE desktop. VisionRuntime drives FaceLandmarker every frame (~30 Hz) and throttles HandLandmarker (~20 Hz) and PoseLandmarker Lite (~15 Hz) independently, emitting a traceable RuntimeStepId event per pipeline stage. extractSignals turns landmarks and blendshapes into a GestureScores vector per state every frame, each score exponentially smoothed (0.52/0.48) before ranking; GestureEngine.update then walks a fixed priority list and requires a candidate to hold for >=120ms (420ms for blank) with a 160ms cooldown between transitions, so the displayed state never chatters. The UI is built on DesktopWindow, a pointer-event-driven docked/floating window abstraction with a resizable camera pane, a live gesture-engine runtime trace, a raw per-state score table, and an Explorer that opens the actual bundled source of every file via Vite's ?raw imports — the source you read is the source running. Reaction media (images/sounds) are drop-in and manifest-mapped, with synthesized two-oscillator fallback tones for unmapped states. `npm run build` runs tsc --noEmit against two tsconfigs before the Vite build, so type errors fail the build itself.",
  ...
  },
  {
    ...
    subtitle: "A personal finance terminal wired to real bank data -- connecting to Lansforsakringar via Enable Banking's Open Banking API with RS256-JWT-signed requests and BankID consent. Transactions are auto-synced every 6 hours and piped through a five-stage categorization pipeline: self-transfer detection -> MCC codes -> keyword rules -> merchant name cache -> Gemini 2.0 Flash batch fallback. Manual overrides propagate backward and forward to all past and future transactions from the same merchant. Budget periods are salary-cycle-aligned -- last salary to next, detected as 18k-30k kr income -- never calendar-month. Recurring payments are auto-detected from transaction history (3+ consistent charges) and rendered as a bills checklist with paid/due/overdue status. Savings goals store cover images in Vercel Blob and auto-sweep the monthly budget surplus each cycle with time-to-goal projections. Insights surfaces month-over-month and week-over-week comparison tables; transfers excluded from all spend totals and budgets. A what-if simulator projects month-end impact of category budget adjustments; adaptive budgeting auto-tightens other categories after large purchases. Anomaly detection flags suspicious payments with [!] ANOMALY. Full retro-CLI aesthetic throughout -- ASCII bars, monospace-only type, terminal-style layout. Stack: Next.js 15 App Router, Drizzle ORM, Neon Postgres, Enable Banking API, Gemini 2.0 Flash, ntfy push notifications, Tailwind CSS 3, Vercel Blob, deployed on Vercel.",
